...
const PatientNotifications = () => {
  ...
  if (loading) {
    return <div className="text-center py-12 text-slate-500">Loading notifications center...</div>;
  }
    ...
    <div className="space-y-6 text-left max-w-3xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-400">Keep updated with alerts, consent authorizations, and health notifications.</p>
        </div>
        {unreadCount > 0 && (
          <button
      ...
      {notifications.length === 0 ? (
        <div className="p-12 text-center bg-slate-900/10 border border-dashed border-slate-800 rounded-3xl">
          <Bell className="w-12 h-12 text-slate-650 mx-auto mb-3" />
          <h3 className="text-sm font-bold text-white">No notifications yet</h3>
          <p className="text-xs text-slate-500 mt-1">
            We will notify you here when doctor connections are made or records decrypted.
          </p>
        </div>
      ) : (
            ...
            <div
              ...
              className={`p-4 rounded-2xl border text-left flex justify-between items-start gap-4 transition-all ${
                notif.read
                  ...
                  : 'bg-brand-500/5 border-brand-500/20 hover:bg-brand-500/10 cursor-pointer'
              }`}
            >
              <div className="flex items-start space-x-3">
                <div className={`p-2 rounded-xl mt-0.5 ${
                  notif.read ? 'bg-slate-800 text-slate-500' : 'bg-brand-500/10 text-brand-400'
                }`}>
